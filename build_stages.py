...
stages = [
    ...
    },
    {
        ...
        "avatar": "PARK_JW",
        "dialogue": "권남훈 파트장이 소명서 승인을 조건으로 장비 배치 관리 대장의 고유 식별 코드를 요구합니다.\n\n[권남훈]: 박 차장의 단독 배치가 원인이라는 정황이 분명합니다. 이의를 제기하고 싶다면 오류 대상인 스토리지 장비가 보관된 랙 식별 번호 2자리(Rack ID)와 결선된 라우터 광포트 번호 2자리(Port ID)를 조합한 4자리 물리 관리 식별자를 제시하십시오.\n\n[박재욱]: 새벽 긴급 상황에서 물리 배치 번호까지 대라니 치졸한 방해 공작입니다. 기계실 인프라 배치 맵과 광 스위치 포트 인터페이스 상태 로그를 대조해 스토리지 랙 번호(2자리)와 스토리지 광 연결 포트 번호(2자리)를 조합한 4자리 장비 식별자(8204)를 확인하십시오.",
    ...
    },
    {
        ...
        "role": "네트워크 담당",
        "avatar": "KIM_JH",
        "dialogue": "김진혁 차장은 당시 본인의 모니터링 접속 환경이 철저하게 보장되었음을 강변합니다.\n\n[김진혁]: 장애가 터진 시각, 저는 휴게실에서 노트북을 가동하고 기계실 랙에 적재된 T5004 라우터 허브 장비의 무선 신호를 수신해 부하 상태를 안전하게 확인 중이었습니다. 통신 끊김은 없었습니다.\n\n하지만 기계실 하드웨어 장치 상세 매뉴얼을 검토하던 중, 김 차장의 알리바이 진술에서 심각한 모순이 탐지됩니다. T5004 장치 도면을 분석하여 그가 주장한 거짓 무선 접속 기술 명칭(영문 소문자 4자)을 식별하십시오.",
    ...
    },
    {
        ...
        "role": "운영총괄",
        "avatar": "KWON_NH",
        "dialogue": "데이터베이스 단절 에러가 특정되었음에도, 권남훈 파트장은 외부 침입 가설을 강하게 부정합니다.\n\n[권남훈]: VDI 보안 규칙상 원격 침입자가 타인의 IP 주소를 도용해 상주하는 행위는 물리적으로 불가능합니다. 로그아웃이 일어나는 순간 주소는 전역 풀로 즉시 회수되어 대기 세션에 재분배되기 때문입니다.\n\n보안망 회수 규칙의 논리적 한계를 입증하기 위해, 사설 IP 주소의 동적 자동 배분을 관장하는 이 네트워크 표준 규약 약어(H C P D)의 아나그램을 풀어 기입하십시오.",
    ...
    },
    {
        ...
        "role": "DBA",
        "avatar": "PARK_JW",
        "dialogue": "김 차장의 서브넷 라우팅 오설정이 스토리지 통신 단절을 일으키고, 그로 인해 결산 DB 데이터가 연쇄 파손되었음이 물리적으로 실증된 국면.\n\n[박용철]: 김진혁! 자네 대체 라우터에 무슨 수정을 가한 건가! 당장 규명하게!\n\n[김진혁]: 단순한 경로 복구 테스트 도중 발생한 일시적 대역 차단이었습니다. 본질적인 WAS 정지와는 별개입니다.\n\n[박용철]: 시끄럽습니다! 즉시 끊어진 블록 저장소 연결부터 수립하십시오. iSCSI 프로토콜 통신 채널 복구를 위해, 해당 스토리지 장치 결선에 기본적으로 할당되어 가동되는 TCP 표준 포트 번호 4자리를 기입하십시오.",
    ...
    },
    {
        ...
        "role": "INVESTIGATION",
        "avatar": "TQO",
        "dialogue": "모든 물리 단서 조각이 일치하며 실마리가 드러납니다.\n서버의 느린 클럭 오차를 보정한 진짜 셧다운 명령 구동 시간은 실제 시간 기준으로 정확히 '02:13'이었습니다.\n\n사건 당일 사용자 망 세션 회수 타임라인을 검토하십시오.\nDBA 박재욱 차장은 백업 업무를 마치고 '02:05'에 공식 로그아웃하여 세션을 해제하고 사용하던 IP를 반납했습니다.\n\n권 파트장이 증명한 DHCP 회수 및 순차 재분배 원칙에 따라, 02:05에 반납된 무결한 사설 IP를 02:13에 즉각 할당받아 악용한 단 한 사람.\n당일 무선(wifi) 신호로 접속 상태를 모니터링했다는 치명적인 기술적 거짓말을 한 진짜 범인의 이름을 터미널에 기입해 규명하십시오.",
