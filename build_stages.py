...
stages = [
    ...
    },
    {
        ...
        "avatar": "PARK_JW",
        "dialogue": "권남훈 파트장이 소명 검증을 위해 뜬금없이 복잡한 물리 보안 관리 식별 코드를 요구합니다.\n\n[권남훈]: 박 차장님의 억지 주장을 정식 소명 티켓으로 접수하려면, 실제 동기화 오류를 일으킨 스토리지 장비의 기계실 물리 수납 랙 번호 2자리(Rack ID)와 해당 장비가 직결된 광스위치 라우터의 물리 포트 번호 2자리(Port ID)를 조합한 4자리 장비 관리 코드를 대셔야 합니다.\n\n[박재욱]: 권 파트장님, 그 복잡한 관리 대장 번호를 제가 새벽에 어떻게 외웁니까! 치졸하게 방해하지 마십시오!\n\n[박재욱]: 조사관님! 기계실 단말기에 기록된 물리 장비 배치 맵과 광 스위치 포트 라우팅 상태 덤프를 해독해 주십시오. 스토리지 랙 번호(2자리)와 스토리지 광 케이블 연결 포트 번호(2자리)를 순서대로 합친 4자리 장비 식별자(8204)를 찾아 주십시오!",
