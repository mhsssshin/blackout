...
stages = [
    {
        ...
        "role": "SYSTEM LOBBY",
        "avatar": "SYS",
        "dialogue": "TQO 터미널 단말기 부팅이 완료되었습니다.<br><br>시스템 분석 모드로 진입하기 위해서는 권남훈 파트장의 인가 코드가 필요합니다.<br>본 화면에는 노출된 단서가 없습니다.<br><br>시스템 구조의 원천 설계 코드(HTML 소스) 내부를 분석해 보십시오.<br>그곳에 개발자가 적어둔 마스터 비밀 번호(KEY) 3자리 숫자가 기록되어 있을 것입니다.",
    ...
    },
    {
        ...
        "role": "PACKET MONITOR",
        "avatar": "NET",
        "dialogue": "권남훈 파트장의 보안 단말기(VDI)로 전송된 내부 사설 포트 전송 패킷이 캡처되었습니다.<br><br>이 데이터는 문자나 숫자를 안전하게 전송하기 위해 64진법 텍스트로 인코딩되어 있습니다.<br>인터넷의 Base64 디코더 등을 활용해 이 값을 원래대로 풀어내면,<br>비밀 사설 포트 번호 4자리가 나옵니다.",
    ...
    },
    {
        ...
        "role": "AUDITOR",
        "avatar": "AUD",
        "dialogue": "권남훈 파트장은 모든 백업 파일들이 규정에 맞춰 정상 권한(rwxr-xr-x)을 가지고 있다고 주장했습니다.<br><br>그러나 스캔 감사 결과, 단 하나의 백업 파일이 권한 관리 부주의로 인해 취약한 설정(rwxr-xr--)으로 배포된 것이 확인되었습니다.<br><br>마우스 스크롤을 이용해 수많은 파일 리스트 중에서 이 잘못 설정된 파일을 찾아내십시오.<br>그 후 아래 규칙에 따라 3자리 8진수 숫자로 변환하여 입력해 주십시오.<br><br>[권한 변환 규칙]<br>r = 4, w = 2, x = 1, - = 0<br>각 문자군(rwx / xr- / xr-)의 숫자를 합산하여 3자리 숫자로 만듭니다.<br>예시: rwxr-xr-x = (4+2+1)(4+0+1)(4+0+1) = 755",
    ...
    },
    {
        ...
        "avatar": "CRT",
        "dialogue": "서버가 완전한 블랙아웃 상태에 빠져 화면 신호가 끊겼습니다! 눈으로는 터미널의 어떤 글자도 읽을 수 없습니다.<br><br>하지만 TQO 긴급 복구 매뉴얼에 따라 단말기 중앙 캔버스 영역에 투명한(검은색) 글씨로 복구 비밀코드가 적혀 있습니다.<br><br>마우스 드래그(또는 전체 선택 Ctrl+A)를 사용해 검은색 배경 뒤에 숨겨진 텍스트를 반전시켜 에러 코드를 확인하십시오.",
    ...
    },
    {
        ...
        "role": "LOG ANALYZER",
        "avatar": "ERR",
        "dialogue": "시스템 강제 중단 여파로 로그 덤프 텍스트에 심각한 노이즈가 발생했습니다.<br><br>하지만 눈을 가늘고 뜨고 자세히 살펴보면, 깨진 문자들 틈바구니 속에 영문 대문자 B, A, D가 순서대로 숨어 있습니다.<br><br>이 글자들을 아스키(ASCII) 10진수 코드로 각각 치환하여 공백 없이 합쳐 입력하십시오.<br><br>[아스키 코드 정보]<br>- B = 66<br>- A = 65<br>- D = 68",
    ...
    },
    {
        ...
        "role": "SECURITY ENGIN",
        "avatar": "SEC",
        "dialogue": "서버를 다운시킨 침입자의 명령어가 세션 암호 로그에 복착되었습니다.<br>침입자는 각 글자를 알파벳 순서상 뒤로 3칸씩 밀어 적는 간단한 암호(시저 암호)를 적용해 두었습니다.<br><br>[복구 힌트: -3 Shift]<br>암호문: vkXwgrzq<br><br>모든 문자를 알파벳 순서상 앞으로 3칸씩 당겨 복구하십시오.<br>(예: d -> a, k -> h, X -> u ...)<br>최종 도출되는 영어 소문자 명령어를 입력하십시오.",
    ...
    },
    {
        ...
        "role": "품질총괄",
        "avatar": "PARK_YC",
        "dialogue": "이보게 TQO! 우리가 이번 차세대 금융 코어 계약에서 약속한 연간 가동률(SLA)이 99.9%일세.<br>1년 365일 중 시스템이 멈춰 있을 수 있는 시간(장애 시간)은 단 0.1% 미만이어야 해.<br><br>그렇다면 연간 가동률 99.9%를 유지하기 위해 허용되는 연간 최대 장애 정지 시간은 총 몇 '분(Minute)'인가?<br><br>계산 오차 방지를 위해 공식으로 나온 약식 기준값(525분)에 우리 부서 내부 통제 계수인 '10'을 곱한 최종 통제 코드를 터미널에 입력하여 내게 보고해 주게!",
    ...
    },
    {
        ...
        "role": "운영총괄",
        "avatar": "KWON_NH",
        "dialogue": "장애가 발생했을 때 단순 땜질 조치를 넘어서야 합니다.<br><br>IT 거버넌스 표준 표준에 따라, 단기 장애 현상(Incident)을 넘어 근본적인 시스템 원인(Problem)을 도출하여 통제하고 품질 지식을 체계적으로 관리하는 글로벌 IT 서비스 관리 모범 기준 프레임워크가 존재합니다.<br><br>이 글로벌 표준 프레임워크의 명칭(영문 소문자 4글자 약어)을 입력하여 프로세스 숙지 여부를 인증하십시오.",
    ...
    },
    {
        ...
        "role": "KNOWLEDGEBASE",
        "avatar": "KB",
        "dialogue": "장애 대응 가이드라인 조회를 위한 사내 지식 정보 포털입니다.<br><br>현재 데이터베이스 서버의 일부 데이터가 파손된 정황이 발견되었습니다.<br>아래의 문서 목록에서 '스토리지 동기화 실패로 인한 DB 파손 대응 매뉴얼'에 해당하는 문서 코드의 숫자 4자리를 찾아 입력하십시오.",
    ...
    },
    {
        ...
        "role": "WAS 담당",
        "avatar": "PARK_JA",
        "dialogue": "새벽에 서버가 셧다운되는 당시에 누군가 제 WAS 콘솔 권한을 악용해 로컬 DB에 수동 인증 조작을 시도한 흔적이 로그에 남아 있었습니다.<br><br>침입자는 오라클 데이터베이스에 직접 명령어를 날리기 위해 전용 데이터베이스 쿼리 접속 CLI 유틸리티를 실행하려 했습니다.<br><br>서버 로그 히스토리에 기록된 이 오라클 질의용 다이렉트 접속 명령어의 정확한 영어 소문자 스펠링을 기입해 주십시오.",
    ...
    },
    {
        ...
        "role": "네트워크 담당",
        "avatar": "KIM_JH",
        "dialogue": "저는 결백합니다 TQO님!<br>장애 당시 기계실 외부 휴게실 소파에서 노트북을 사용해 T5004 공유기의 무선 신호를 안정적으로 송수신하며 서버들을 원격으로 정상 모니터링하고 있었습니다!<br><br>제 단말기에는 무선 접속 안테나가 원활하게 표시되어 있었습니다.",
    ...
    },
    {
        ...
        "role": "서버 담당",
        "avatar": "LEE_JH",
        "dialogue": "서버 자원은 풍부하게 할당해 두어 넉넉한 튜닝 상태였습니다.<br>물리적 스펙 부족으로 중단되었을 리는 절대 없습니다. 충분히 여유가 있었을 텐데요?",
    ...
    },
    {
        ...
        "role": "품질총괄",
        "avatar": "PARK_YC",
        "dialogue": "서버가 멈추기 직전에 데이터베이스에 심각한 블록 손상 오류가 감지되었네.<br><br>오라클 데이터베이스의 물리적 파일 파손을 일으키는 고유한 장애 에러 패턴을 정규표현식(Regex) 필터로 검출해 내려 하네.<br>아래 필터에 일치하는 로그 에러 코드 명칭을 찾아 영어 소문자로 기입해 주게.",
    ...
    },
    {
        ...
        "role": "운영총괄",
        "avatar": "KWON_NH",
        "dialogue": "인프라 보안 규정상 세션이 종료되면 할당된 사설 IP는 즉시 반납 처리됩니다.<br><br>이후 새로 세션을 맺는 타인에게 해당 IP 주소가 동적으로 자동 재분배되도록 관리해 주는 표준 네트워크 규약이 존재합니다.<br>철자가 뒤섞여 있는 프로토콜 문자 'H C P D'의 순서를 바르게 나열해 표준 약어(소문자 4자)를 도출하십시오.",
    ...
    },
    {
        ...
        "role": "ROUTING ERROR",
        "avatar": "NET",
        "dialogue": "김진혁 차장이 장애 발생 30분 전,<br>라우터 서브넷 마스크 대역을 조작하면서 실수로 CIDR 마스크 범위를 `/28` 로 지정해 버렸습니다.<br>이로 인해 스토리지와 서버 대역이 끊겼습니다.<br><br>서브넷 마스크 `/28` 등급 대역에서 서브넷 대표 주소(Network ID)와 브로드캐스트 주소(Broadcast)를 제외하고<br>장비에 유효하게 수동 분배해 줄 수 있는 최대 IP(호스트) 개수를 정수 숫자로 계산하십시오.",
    ...
    },
    {
        ...
        "role": "STORAGE LOG",
        "avatar": "SAN",
        "dialogue": "서브넷 격리 사고 이후 스토리지(SAN)와 데이터베이스 간의 블록 데이터 통신 연결이 오프라인되었습니다.<br><br>스토리지와 서버 간의 디스크 읽기/쓰기 채널을 재연결하고 iSCSI 프로토콜 통신을 정상화시키기 위해, 네트워크 장비가 사용하는 기본 표준 TCP 포트 번호 4자리를 기입해 주십시오.",
    ...
    },
    {
        ...
        "role": "서버 담당",
        "avatar": "LEE_JH",
        "dialogue": "아차... TQO님, 제가 이번 서버 증설 당시에 시간 자동 동기화(NTP) 스케줄을 꺼두는 바람에,<br>현재 WAS 물리 서버의 시계가 실제 시간(VDI 표준 시간)보다 정확히 '15분 느리게' 흘러가고 있는 것을 뒤늦게 확인했습니다.<br><br>WAS 내부 로그기록 상 강제 셧다운이 수행된 기록 시각은 '01:58'입니다.<br>이 기록에 15분 오차를 정상적으로 보정한 실제 세상의 정확한 표준 시간(HHMM)을 4자리 숫자로 계산하십시오.",
    ...
    },
    {
        ...
        "role": "INVESTIGATION",
        "avatar": "TQO",
        "dialogue": "모든 인프라 모순을 추적한 결과 사건 당일의 실제 표준 시간 타임라인이 규명되었습니다.<br><br>- 01:45 : DBA 박재욱 차장, backup.sh 백업 스케줄 자동 기동 시작<br>- 02:05 : 백업 프로세스 정상 종료. DBA 세션 로그아웃 및 VDI 사용 IP 반납 완료<br>- 02:13 : (실제 시각 기준) 코어 WAS 서버 강제 중단 명령어 실행 및 중단 장애 발생<br><br>운영 규정에 따르면, DBA가 02:05에 로그아웃하고 반납한 IP는 비어 있는 상태였습니다. 이 빈 IP를 가로채 02:13에 은밀하게 서버에 침투해 중단 명령을 내리고 DBA 박 차장에게 누명을 씌우려 한 진범의 이름을 기재하십시오.<br><br>[용의자 후보]<br>박용철, 권남훈, 김진혁, 박재욱, 박주암, 이재헌",
    ...
    },
    {
        ...
        "role": "품질총괄",
        "avatar": "PARK_YC",
        "dialogue": "훌륭하네 TQO! 김진혁 차장이 본인의 라우팅 작업 실수를 감추기 위해 WAS 서버를 다운시키고 DBA에게 혐의를 뒤집어 씌우려 한 사실이 명백히 입증되었군!<br><br>자네 덕분에 장애 관제 티켓 최종 결재 단계까지 도달했네. 티켓을 최종 승인 종결(Incident Closed) 처리하고 시스템을 정상 롤백하기 위해 마지막 확인 단어로 영문 소문자 [ done ]를 기입해 주게!",
