...
stages = [
    ...
    },
    {
        ...
        "role": "품질총괄 (예민한 리더)",
        "avatar": "avatar_park_yc.png",
        "narrative": "권남훈 파트장의 장담과 달리, 파일 배포 프로세스 상에 허점이 감지됩니다.\n권 파트장은 표준 rwxr-xr-x(755) 권한으로 배포했으니 무결하다고 주장합니다.",
        "dialogue": "권 파트장의 말은 신뢰하기 어렵습니다.\n즉시 스토리지의 디렉토리 권한 감사 덤프를 전수 스캔하십시오.\n\n관리 소홀로 인해 쓰기 권한이 허용된 비정상 파일(rwxr-xr--)을 가려내야 합니다.\n찾아내서 수치값으로 변환해 제출하십시오.",
    ...
    },
    {
        ...
        "dialogue": "누군가가 강제로 단말기 모니터 신호를 차단해 분석을 방해하려는 구동으로 추정됩니다.\n\n터미널 장치의 비상 자체 안전 장치에 의해, 화면 중앙 캔버스 어딘가에 마스크 처리된 복구 코드가 숨겨져 있습니다.\n\n드래그(Ctrl+A)를 수행해 코드를 읽어내십시오.",
    ...
    },
    {
        ...
        "dialogue": "셧다운 직전인 02시 13분, 누군가가 root 권한 세션을 획득해 프로세스 가동을 강제로 차단하고 커널을 Halt시키는 바이너리를 실행한 흔적이 포착되었습니다.\n\n공격자가 서버 자체를 완전히 정지시켜 물리 분석을 무력화하기 위해 실행한 UNIX/Linux 표준 시스템 종료 명령어(영문 소문자 8자)를 파악하십시오.",
    ...
    },
    {
        ...
        "dialogue": "정형화된 절차를 우회하는 조사는 명백한 규정 위반입니다.\n\n장애 상황 관리 시에는 임시 조치 단계부터 근본 문제(Problem)를 정식 식별하여 등록하는 모범 체계가 요구됩니다.\n\n품질 보증 표준 프로세스를 이수하기 위해, 사내 거버넌스가 따르는 정식 IT 서비스 관리 프레임워크의 영문 소문자 4글자 약어를 입력하여 수사 무결성을 확보하십시오.",
    ...
    },
    {
        ...
        "dialogue": "소명 티켓 접수를 위해서는 실제 동기화 오류를 일으킨 스토리지 장비가 보관된 랙 식별 번호 2자리(Rack ID)와 결선된 라우터 광포트 번호 2자리(Port ID)를 조합한 4자리 장비 관리 코드를 대셔야 합니다.\n\n이 새벽에 그런 복잡한 대장 번호를 다 외우는 것은 불가능합니다.\n기계실 단말기에 기록된 물리 장비 배치 맵과 광 스위치 포트 인터페이스 상태 로그를 대조해 4자리 장비 식별자(8204)를 확인하십시오.",
    ...
    },
    {
        ...
        "narrative": "네트워크 담당 김진혁 차장이 개입하여 박재욱 차장의 임의 수동 조작 혐의를 주장하고 나섭니다.\n박 차장은 수동 가동 사실이 없음을 해명합니다.",
        "dialogue": "스토리지 장애를 핑계 대고 있지만, 본질은 박 차장이 임의로 스크립트를 수동 가동해 시스템을 과부하시킨 겁니다.\n로그가 명백히 증명하고 있습니다.\n\n의혹을 규명하기 위해, 타겟 서버의 크론탭(Crontab) 작업 설정 데이터 상 배치 스크립트가 기동되도록 자동 스케줄링되어 있던 시각(24시간 형식 4자리 HHMM)을 도출하십시오.",
    ...
    },
    {
        ...
        "dialogue": "단말기 명령어 히스토리를 뜯어보니 셧다운 직전에 타인의 무단 침투 정황이 관측됩니다.\n\n02시 13분경, 누군가 WAS 계정을 불법 도용해 특정 데이터베이스 커널 접속 유틸리티를 기동했습니다.\n\n침입자가 DB 전체를 오프라인 상태로 전환하여 동기화 장애를 유발하고 인프라를 깨뜨리기 위해 실행한 이 데이터베이스 전용 질의 클라이언트 명령어(영문 소문자)를 파악하십시오.",
    ...
    },
    {
        ...
        "dialogue": "장애가 터진 시각, 자신은 휴게실에서 노트북을 가동하고 기계실 랙에 적재된 라우터 허브 장비의 무선 신호를 수신해 부하 상태를 안전하게 확인 중이었다고 설명합니다. 통신 끊김은 없었다는 주장입니다.\n\n하지만 기계실 하드웨어 장치 상세 매뉴얼을 검토하던 중, 김 차장의 알리바이 진술에서 심각한 모순이 탐지됩니다. 장치 도면을 분석하여 그가 주장한 거짓 무선 접속 기술 명칭(영문 소문자 4자)을 식별하십시오.",
    ...
    },
    {
        ...
        "dialogue": "일시적 포트 통신 실패에 따른 데이터베이스 파손 정황을 정확하게 짚고 가야 합니다.\n즉시 스토리지 단절 당시 연쇄적으로 발생한 디스크 물리 무결성 실패 로그부터 해독하십시오.\n\n제시된 정규표현식(Regex) 검출 필터를 로그 덤프에 매칭해, 디스크 파손을 나타내는 오라클 내부 치명적 에러 코드를 소문자로 기입하십시오.",
    ...
    },
    {
        ...
        "dialogue": "VDI 보안 규정상 외부 침입자가 타인의 IP 주소를 도용하는 행위는 논리적으로 차단되어 있습니다.\n\n사용자가 세션을 종료하는 즉시 할당되어 있던 사설 IP 주소는 풀(Pool)로 회수되며, 이후 새로 로그인하는 다른 사람에게 즉각 순차 재배포되기 때문입니다.\n\n보안망 회수 규칙의 한계를 검증하기 위해, 사설 IP 주소의 동적 자동 배분을 관장하는 이 네트워크 표준 규약 약어(H C P D)의 아나그램을 풀어 기입하십시오.",
    ...
    },
    {
        ...
        "role": "SYSTEM SYSTEM",
        "avatar": "NET",
        "narrative": "네트워크 스위치 콘솔 로그를 상세 조회하던 중, 스토리지 관리 권한에서 중대한 작업 조작 실수의 흔적이 발견됩니다.\n김진혁 차장이 장애 발생 30분 전, 백업 보강용 라우팅 대역 테이블을 조작하다가 실수로 CIDR 마스크 대역 폭을 `/28` 로 잘못 격리해 스토리지 통신 연결이 차단된 것입니다.",
    ...
    },
    {
        ...
        "role": "DBA",
        "avatar": "avatar_park_jw.png",
        "narrative": "김 차장의 서브넷 라우팅 오설정이 스토리지 통신 단절을 일으키고, 그로 인해 결산 DB 데이터가 연쇄 파손되었음이 물리적으로 실증되었습니다.\n박용철 파트장이 분개하자 김 차장은 일시적인 복구 테스트 중 발생한 설정 편차라며 얼버무립니다.",
        "dialogue": "끊겨버린 스토리지 연결 경로의 즉각적인 수동 복구가 요구됩니다.\n\niSCSI 프로토콜 통신 채널 복구를 위해, 해당 스토리지 장치 결선에 기본적으로 할당되어 가동되는 TCP 표준 포트 번호 4자리를 기입하십시오.",
    ...
    },
    {
        ...
        "dialogue": "인프라 증설 단계에서 시간 동기화 데몬(NTP) 동작 설정을 수동으로 누락한 결과,\nWAS 물리 서버의 시스템 내부 시간이 실제 세상 표준시보다 정확히 '15분 느리게' 흘러가고 있음이 파악되었습니다.\n\nWAS의 감사 기록 상 강제 중단 명령어(`sqlplus`)가 구동된 서버 타임 스탬프는 '01:58'로 확인됩니다.\n실제 세상의 실제 셧다운 구동 표준 시각(24시간 형식 4자리 HHMM)을 도출하십시오.",
    ...
    },
    {
        ...
        "role": "INVESTIGATION",
        "avatar": "avatar_park_yc.png",
        "narrative": "모든 물리 단서 조각이 일치하며 실마리가 드러납니다.\n서버의 느린 클럭 오차를 보정한 진짜 셧다운 명령 구동 시간은 실제 시간 기준으로 정확히 '02:13'이었습니다.\nDBA 박재욱 차장은 '02:05'에 공식 로그아웃하여 IP를 반납했습니다.",
        "dialogue": "DHCP 순차 재분배 원칙에 따라, 02:05에 반납된 무결한 사설 IP를 02:13에 즉각 할당받아 악용한 인물이 좁혀집니다.\n\n당일 무선(wifi) 신호로 접속 상태를 모니터링했다는 치명적인 기술적 거짓말을 늘어놓은 진짜 범인의 이름을 기입하십시오.",
    ...
    },
    {
        ...
        "role": "품질총괄 (예민한 리더)",
        "avatar": "avatar_park_yc.png",
        "narrative": "진범의 정체가 최종 확인되었습니다.\nOSPF 라우팅 오설정을 숨기기 위한 김진혁 차장의 전말이 드러납니다.\n결국 공격자가 내세운 무선 모니터링이라는 하드웨어적 모순이 자가당착을 유발했습니다.",
        "dialogue": "성공적인 장애 대응 및 음모 규명이 완료되었습니다.\n\n최종 승인 확인 키 [ done ]를 기입하여 이 새벽의 장애 인시던트 티켓을 공식 클리어하고 종결 처리하십시오.",
        ...
        <div class="dashboard-grid" id="stage-container" data-stage="{stage_num}" data-verify="{verify_hash}">
            ...
            <main class="main-stage-panel">
                ...
                <div class="story-container">
                    ...
                    <div class="narrative-panel" style="background: rgba(0, 229, 255, 0.03); border-left: 3px solid var(--accent-blue); padding: 12px 16px; margin-bottom: 20px; font-size: 0.9rem; line-height: 1.6; color: var(--text-muted); border-radius: 0 4px 4px 0; font-family: var(--font-sans); white-space: pre-wrap;">
                        {narrative}
                    </div>
